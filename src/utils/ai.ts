...
function stickerPrompt(friend: Friend): string {
  ...
  if (friend.stickerEnabled === false || custom.length === 0) {
    lines.push(
      friend.stickerEnabled === false
        ? '【表情使用禁令（最高优先级）】你不可以使用任何表情包、表情图、emoji、颜文字、表情符号，回复必须是纯文字。'
        : '【表情包使用】对方没有添加自定义表情包，你不可以发送[表情:xx]格式的标记，也不可以输出图片链接。'
    )
  } else {
    lines.push('【表情包使用】你可以用表情让聊天更生动。发送表情的固定格式：[表情:名字]，渲染时会被替换成表情图片。')
    lines.push(`可用的表情名字（任选）：${custom.map((s) => s.meaning).join('、')}。这些意思必须一字不差地使用。`)
    lines.push('表情使用规则：像真人一样自然地穿插，一次回复最多1个表情，开心、惊讶、无语等情绪强烈时用，平时多数纯文字。标记必须放在整条消息的最末尾，标记后面禁止再加任何标点符号或文字，不要自己发明别的表情写法，不要输出图片链接。')
  }
  lines.push(
  ...
  lines.push(
    '【文字图片功能】你可以用 [文字图片:内容] 发送一张白底黑字的文字图片（把一段话做成图片发出去），适合郑重表白、道歉、强调很重要的话、写一段心里话等场合，内容10-40字，一条回复最多1个，标记必须放在整条消息的最末尾，标记后面禁止再加任何标点符号或文字，方括号格式必须严格遵守。'
  )
  lines.push(
  ...
  lines.push(
    '【消息格式说明】对方消息中的 [位置:xx] 表示对方共享了 TA 的位置；[图片] 表示对方发了一张图片；"文字图片"表示对方发了白底文字图片，消息文字就是图片上的内容；[红包:xx|金额] 表示对方给你发了一个微信红包；[转账:金额|备注] 表示对方给你转了一笔钱。收到红包或转账时，用文字表达感谢即可。'
  )
  return lines.join('\n')
}
