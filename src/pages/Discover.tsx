...
function Shape({ shape }: { shape: Item['shape'] }) {
  const s = { stroke: '#fff', strokeWidth: 1.8, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const, fill: 'none' }
  switch (shape) {
    case 'star':
      return (
    ...
    case 'game':
      return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
          ...
          <circle cx="17.5" cy="13.5" r="0.9" fill="#fff" stroke="none" />
        </svg>
      )
    case 'mini':
      return (
        ...
        {GROUPS.map((group, gi) => (
          <div className="list-group" key={gi}>
            {group.map((item) => (
              ...
              >
                <div className="row-icon row-icon-grad" style={{ background: item.bg }}>
